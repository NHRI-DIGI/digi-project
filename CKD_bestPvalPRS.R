...
urPhen <- "CKD" # Type phenotype here
phenotype <- read.table("/Users/kenny/Desktop/Laptop_input/CKD_12_target.txt", header=T) %>%
  mutate(CKD_12 = CKD_12 - 1)
...
for(i in p.threshold){
  pheno.prs <- paste0('/Users/kenny/Desktop/Laptop_input/FinalResults/CKD/pval_th.', i, ".profile") %>%
    fread(.) %>%
    .[,c("FID", "IID", "SCORE")] %>%
...
res <- read.table(paste0('/Users/kenny/Desktop/Laptop_input/FinalResults/CKD/pval_th.',bestCutOff,".profile"), header=T)
p <- phenotype
p2 <- select(p,FID,IID,CKD_12)
...
ggsave(plot=gd, filename=paste0("/Users/kenny/Desktop/Laptop_output/CKD_DensityPlot.tiff"), height = 4, width = 6)
...
abline(v=0.01, col="BLUE", lty=2) # THRESHOLD=missing_rate
...
if (length(imiss_het[imiss_het$F_MISS>0.01 |imiss_het$Het_propo>m2 |imiss_het$Het_propo<m1,'FID'])!=0){
text(x=imiss_het[imiss_het$F_MISS>0.01 |imiss_het$Het_propo>m2 |imiss_het$Het_propo<m1,'F_MISS' ],y=imiss_het[imiss_het$F_MISS>0.01 |imiss_het$Het_propo>m2 |imiss_het$Het_propo<m1,'Het_propo' ],labels=imiss_het[imiss_het$F_MISS>0.01 |imiss_het$Het_propo>m2 |imiss_het$Het_propo<m1,'FID'],cex=0.9, font=2, pos=4)
}
dev.off()
...
abline(h=mean(imiss_het$Het_propo)
       +(3*sd(imiss_het$Het_propo)),col="RED",lty=2)
##Missing Data Thresholds (Vertical Line)
abline(v=0.015, col="BLUE", lty=2) # THRESHOLD=missing_rate
title("Distribution of missing and heterozygosity rate")

m1<-mean(imiss_het$Het_propo)-(3*sd(imiss_het$Het_propo))
m2<-mean(imiss_het$Het_propo)+(3*sd(imiss_het$Het_propo))

if (length(imiss_het[imiss_het$F_MISS>0.015 |imiss_het$Het_propo>m2 |imiss_het$Het_propo<m1,'FID'])!=0){
text(x=imiss_het[imiss_het$F_MISS>0.015 |imiss_het$Het_propo>m2 |imiss_het$Het_propo<m1,'F_MISS' ],y=imiss_het[imiss_het$F_MISS>0.015 |imiss_het$Het_propo>m2 |imiss_het$Het_propo<m1,'Het_propo' ],labels=imiss_het[imiss_het$F_MISS>0.015 |imiss_het$Het_propo>m2 |imiss_het$Het_propo<m1,'FID'],cex=0.9, font=2, pos=4)
}
