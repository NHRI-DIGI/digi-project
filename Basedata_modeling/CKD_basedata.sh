...
phenoarray=CKD_24
merge_dir='/NovaSeq_128/Kenny/CKD/test/Input/Basedata/VCF_merge/'
...
# Indiviual QC
${tool_dir}plink --vcf process/nodup_chr_xym_"$phenoarray".vcf --make-bed --out process/indQC
${tool_dir}plink --bfile process/indQC --missing --out process/miss_indQC
${tool_dir}plink --bfile process/indQC --het --out process/het_indQC
Rscript ${tool_dir}indQC.r
...
${tool_dir}plink --vcf process/nodup_chr_xym_"$phenoarray".vcf --remove fail_indQC.txt --make-bed  --out process/indQC_nodup_chr_xym_"$phenoarray"
${tool_dir}plink --bfile process/indQC_nodup_chr_xym_"$phenoarray" --recode vcf-iid --out process/indQC_nodup_chr_xym_"$phenoarray"
sed -i '/^##/d' process/indQC_nodup_chr_xym_"$phenoarray".vcf

# Add phenotype info (Every pheno output call "$phenoarray"_p)
${tool_dir}plink --vcf process/indQC_nodup_chr_xym_"$phenoarray".vcf --pheno ${pathway}Input/Laptop_input/"$phenoarray"_base.txt --pheno-name $phenoarray --make-bed --out $phenoarray/"$phenoarray"_p
...
Rscript ${tool_dir}snpQCreport_base.r

#Association Test
${tool_dir}plink --bfile $phenoarray/"$phenoarray"_p --assoc --allow-no-sex --out $phenoarray/chi
...
cd $phenoarray
${tool_dir}plink --bfile "$phenoarray"_p --recode vcf-iid --out ef_al_process/"$phenoarray"_p
cd ef_al_process
...
${tool_dir}plink --vcf cont.vcf --make-bed --out cont
${tool_dir}plink --vcf case.vcf --make-bed --out case
${tool_dir}plink --bfile cont --freq --out freq_cont
${tool_dir}plink --bfile case --freq --out freq_case
...
perl ${tool_dir}ef_ad.pl #otput: ef_ambRm_chi.assoc
cd ../../
